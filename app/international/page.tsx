...
// const newsCategories = ["Macroeconomia", "FED", "USA", "Argentina", "Finanzas"]
...
//   const [news, setNews] = useState<NewsItem[]>([])
//   const [selectedCategory, setSelectedCategory] = useState("Macroeconomia")
//   const [currentPage, setCurrentPage] = useState(1)
//   const [totalPages, setTotalPages] = useState(1)
//   const [selectedNews, setSelectedNews] = useState<NewsItem | null>(null)
...
//     async function fetchNews() {
//       try {
//         const response = await fetch(`/api/news?category=${selectedCategory}&page=${currentPage}`)
//         if (!response.ok) throw new Error("Failed to fetch news")
//         const data = await response.json()
//         setNews(data.news)
//         setTotalPages(data.totalPages)
...
//   const chartData = {
//     labels: cpiData.slice(-12).map((d) => new Date(d.date).toLocaleDateString()),
//     datasets: [
//       {
//         label: "CPI",
//         data: cpiData.slice(-12).map((d) => d.value),
...
//               Volver
//             </Link>
//             <h1 className="text-2xl font-bold">Sección Internacional</h1>
//           </div>
//           <ThemeToggle />
//         </div>
//       </header>
...
//             <CardHeader>
//               <CardTitle>Indicadores Económicos (Últimos 12 meses)</CardTitle>
//             </CardHeader>
//             <CardContent>
//               <Line options={chartOptions} data={chartData} />
//             </CardContent>
...
//                     <TableCell>{new Date(cpiData[cpiData.length - 1]?.date).toLocaleDateString()}</TableCell>
//                   </TableRow>
//                   <TableRow>
//                     <TableCell>PCE</TableCell>
//                     <TableCell>{pceData[pceData.length - 1]?.value.toFixed(2)}</TableCell>
//                     <TableCell>{new Date(pceData[pceData.length - 1]?.date).toLocaleDateString()}</TableCell>
//                   </TableRow>
//                   <TableRow>
//                     <TableCell>Federal Funds Rate</TableCell>
//                     <TableCell>{fedFundsRate[fedFundsRate.length - 1]?.value.toFixed(2)}%</TableCell>
//                     <TableCell>{new Date(fedFundsRate[fedFundsRate.length - 1]?.date).toLocaleDateString()}</TableCell>
//                   </TableRow>
//                 </TableBody>
//               </Table>
//             </CardContent>
