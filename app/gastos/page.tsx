...
export default function Gastos() {
  ...
  const agregarGasto = (tarjetaIndex: number) => {
    const nuevasTarjetas = [...tarjetas];
    nuevasTarjetas[tarjetaIndex].gastos.push({
  ...
  const actualizarOtroGasto = (
    ...
    if (campo === "nombre") {
      nuevosOtrosGastos[index].nombre = valor as string;
    } else if (campo === "monto") {
      nuevosOtrosGastos[index].monto = valor as number;
    } else if (campo === "pagado") {
      nuevosOtrosGastos[index].pagado = valor as boolean;
    }
  ...
  const agregarOtroGasto = () => {
    setOtrosGastos([
      ...otrosGastos,
      { nombre: "Nuevo Gasto", monto: 0, pagado: false },
    ]);
  };
  ...
  const totalGastosTarjetas = tarjetas.reduce(
    (total, tarjeta) =>
      total +
      tarjeta.gastos.reduce(
        (sum, gasto) => sum + gasto.montos[mesSeleccionado],
        0
      ),
    0
  );
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="flex items-center gap-4 mb-6 ml-4">
        <BackButton href="/" />
        <h1 className="text-3xl font-bold">Seguimiento de Gastos</h1>
      </div>
            ...
            <CardHeader className="flex flex-row items-center justify-between">
              <CardTitle className="flex items-center">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    actualizarNombreTarjeta(tarjetaIndex, e.target.value)
                  }
                  className="w-48"
                />
              </CardTitle>
              <Button
                variant="outline"
                size="icon"
                onClick={() => agregarGasto(tarjetaIndex)}
              >
                    ...
                    <TableRow key={gastoIndex}>
                      ...
                      <TableCell>
                        <Button
                          ...
                          size="icon"
                          onClick={() =>
                            eliminarGasto(tarjetaIndex, gastoIndex)
                          }
                        >
                ...
                <TableFooter>
                  <TableRow>
                    ...
                    <TableCell className="text-right font-bold">
                      {tarjeta.gastos.reduce(
                        (total, gasto) => total + gasto.montos[mesSeleccionado],
                        0
                      )}
                    </TableCell>
              ...
              <TableRow key={index}>
                <TableCell>
                  <Input
                    value={gasto.nombre}
                    onChange={(e) =>
                      actualizarOtroGasto(index, "nombre", e.target.value)
                    }
                    className="w-32"
                  />
                </TableCell>
                ...
                <TableCell>
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      actualizarOtroGasto(index, "pagado", checked)
                    }
                  />
                </TableCell>
                <TableCell>
                  <Button
                    variant="destructive"
                    size="icon"
                    onClick={() => eliminarOtroGasto(index)}
                  >
                    <Trash2 className="h-4 w-4" />
                  </Button>
                </TableCell>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            <CardTitle>
              Distribución de Gastos ({meses[mesSeleccionado]})
            </CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle>
              Porcentaje de Gastos sobre Sueldo ({meses[mesSeleccionado]})
            </CardTitle>
          </CardHeader>
          <CardContent>
